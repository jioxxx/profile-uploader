...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  elseif (!preg_match('/^[a-zA-Z0-9_\-]+$/', $username))
    $errors['username'] = 'Only letters, numbers, - and _ allowed.';
  if (!$email)
  ...
  if (!$errors) {
    ...
    $stmt = $db->prepare("INSERT INTO profiles (name,username,email,headline,bio,location,website,avatar,github,twitter,linkedin,skills,verified,verification_token)
                               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0,?)");
    $stmt->execute([$name, $username, $email, $headline, $bio, $location, $website, $avatar, $github, $twitter, $linkedin, $skills, $verification_token]);
    ...
    notify_profile_verification($new_profile, $verification_token);
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="card">
        ...
        <div class="av-box" onclick="document.getElementById('avatar').click()">
          <img id="av-preview" src="https://ui-avatars.com/api/?name=You&background=2d6a4f&color=fff&size=144"
            alt="preview">
          <div class="hint">Click to upload &mdash; JPG, PNG, GIF, WEBP · max 2MB</div>
        </div>
        <input type="file" id="avatar" name="avatar" accept="image/*" style="display:none">
        <?php if (isset($errors['avatar'])): ?>
          <div class="err"><?= $errors['avatar'] ?></div><?php endif ?>
      </div>
        ...
        <div class="row">
          <div class="fg">
            ...
            <?php if (isset($errors['name'])): ?>
              <div class="err"><?= $errors['name'] ?></div><?php endif ?>
          </div>
          <div class="fg">
            ...
            <?php if (isset($errors['username'])): ?>
              <div class="err"><?= $errors['username'] ?></div><?php endif ?>
          </div>
        </div>
        <div class="fg">
          ...
          <?php if (isset($errors['email'])): ?>
            <div class="err"><?= $errors['email'] ?></div><?php endif ?>
        </div>
        <div class="fg">
          <label for="headline">Headline</label>
          <input type="text" id="headline" name="headline" value="<?= htmlspecialchars($old['headline'] ?? '') ?>"
            placeholder="Full-Stack Developer · Coffee Lover">
        </div>
        <div class="fg">
          <label for="bio">Bio</label>
          <textarea id="bio" name="bio"
            placeholder="A little about yourself…"><?= htmlspecialchars($old['bio'] ?? '') ?></textarea>
        </div>
        <div class="row">
          <div class="fg">
          ...
          <div class="fg">
            ...
            <input type="url" id="website" name="website" value="<?= htmlspecialchars($old['website'] ?? '') ?>"
              placeholder="https://yoursite.com">
            <?php if (isset($errors['website'])): ?>
              <div class="err"><?= $errors['website'] ?></div><?php endif ?>
          </div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="fg">
          <label>GitHub</label>
          <div class="prefix-wrap"><span class="prefix">github.com/</span><input type="text" name="github"
              value="<?= htmlspecialchars($old['github'] ?? '') ?>" placeholder="username"></div>
        </div>
        <div class="fg">
          <label>Twitter / X</label>
          <div class="prefix-wrap"><span class="prefix">x.com/</span><input type="text" name="twitter"
              value="<?= htmlspecialchars($old['twitter'] ?? '') ?>" placeholder="username"></div>
        </div>
        <div class="fg">
          <label>LinkedIn</label>
          <div class="prefix-wrap"><span class="prefix">linkedin.com/in/</span><input type="text" name="linkedin"
              value="<?= htmlspecialchars($old['linkedin'] ?? '') ?>" placeholder="username"></div>
        </div>
      </div>
