...
if ($search) {
  $stmt = $db->prepare("SELECT * FROM profiles WHERE (name LIKE ? OR headline LIKE ? OR skills LIKE ?) AND (verified = 1 OR verified IS NULL) ORDER BY created_at DESC LIMIT $limit OFFSET $offset");
  $like = "%$search%";
  $stmt->execute([$like, $like, $like]);
  $countStmt = $db->prepare("SELECT COUNT(*) FROM profiles WHERE (name LIKE ? OR headline LIKE ? OR skills LIKE ?) AND (verified = 1 OR verified IS NULL)");
  $countStmt->execute([$like, $like, $like]);
} else {
  $stmt = $db->prepare("SELECT * FROM profiles WHERE verified = 1 OR verified IS NULL ORDER BY created_at DESC LIMIT $limit OFFSET $offset");
  $stmt->execute();
  $countStmt = $db->prepare("SELECT COUNT(*) FROM profiles WHERE verified = 1 OR verified IS NULL");
  $countStmt->execute();
}
