...
if (!$p) {
  ...
  die("<p style='font-family:sans-serif;padding:2rem'>Profile not found. <a href='index.php'>Go home</a></p>");
}
...
// Send profile visit notification (skip if viewing own profile)
// Note: We don't have session/auth, so we send on every external visit
// In a real app, you'd check if the current viewer is the profile owner
$already_notified = false;
if ($visitor_ip && $visitor_ip !== '127.0.0.1' && $visitor_ip !== '::1') {
  // Simple check: don't notify on page refreshes using session
  session_start();
  $notify_key = 'notified_' . $p['id'];
...
<body>
  ...
  <div class="hero-pd">
    <div class="hero-inner">
      ...
      <div class="meta-row">
        ...
        <?php endif ?>
        <?php if ($p['website']): ?>
          <span class="meta-item"><a href="<?= htmlspecialchars($p['website']) ?>" target="_blank" rel="noopener">🔗
              <?= htmlspecialchars(parse_url($p['website'], PHP_URL_HOST)) ?></a></span>
        <?php endif ?>
      </div>
    ...
    <div class="pcard">
      ...
      <div class="detail-row">
        ...
        <?php if ($p['website']): ?>
          <div class="drow">
            ...
            <a href="<?= htmlspecialchars($p['website']) ?>" target="_blank"
              rel="noopener"><?= htmlspecialchars($p['website']) ?></a>
          </div>
        <?php endif ?>
        <?php if ($p['location']): ?>
          <div class="drow">
        ...
        <div class="drow" style="color:var(--muted);font-size:.8rem">
          Member since <?= date('F Y', strtotime($p['created_at'])) ?>
        </div>
      </div>
    </div>
  ...
  <footer>ProfileGen &mdash; Plain PHP + MySQL &middot; <?= date('Y') ?></footer>
</body>
